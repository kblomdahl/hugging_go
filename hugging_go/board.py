...
class Board:
    def __init__(self, komi=5.5):
        self.vertices = [0 if i < 361 else -1 for i in range(368)]
        self.next_vertex = list(range(368))
        self.komi = komi
        self.zobrist_hash = 0

        # forbid the empty board state for occuring again
        self.zobrist_hashes = [0] * 8
        self.zobrist_hashes_index = 0

    def _has_one_liberty(self, index):
    ...
    def _has_two_liberty(self, index):
        """ Returns a non-zero value if the group that the given vertex belongs
    ...
    def _is_valid(self, color, index):
    ...
    def _connect_with(self, index, other):
        ...
        #   a -> 2 -> 3 -> 1 -> b -> c -> a
        #
        index_prev = self.next_vertex[index]
        other_prev = self.next_vertex[other]

        self.next_vertex[other] = index_prev
        self.next_vertex[index] = other_prev

    def _place(self, color, index):
    ...
    def place(self, color, vertex):
        """ Place a stone that belongs to player `color` on the given vertex.
        This function assume that the given move is valid according to the
        `is_valid` function. """

        self._place(int(color), 19 * vertex.y + vertex.x)

    def _get_num_liberties(self, index):
